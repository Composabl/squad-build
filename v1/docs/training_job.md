...
Exactly one child key must be present. v1 supports `local`, `docker`, `kubernetes`, `kubernetes_job`, and `amesa`.

### `local`

The sim is already running at a known address.
...
The trainer spawns the sim as a Docker container.
...
| Field      | Default  | Description                           |
| ---------- | -------- | ------------------------------------- |
| `image`    | required | Docker image tag                      |
| `protocol` | `"grpc"` | Transport protocol                    |
| `registry` | —        | Optional private registry credentials |

### `kubernetes`

Submits sim pods to a Kubernetes cluster.
...
| `sim_cpu`        | `"500m"`         | CPU limit for sim pods                 |
| `sim_gpu`        | `"0"`            | GPU limit for sim pods                 |
| `sim_memory`     | `"256Mi"`        | Memory limit for sim pods              |

### `kubernetes_job`

**Required for `amesa job start`.** Runs sim pods as a Kubernetes Job.
...
```json
{
  "target": {
    "kubernetes_job": {
      "image": "my-sim:latest",
      "regcred": "my-registry-secret",
      "namespace": "amesa-train",
      "namespace_sims": "amesa-sims",
      "node_selector_agentpool": "training-pool"
    }
  }
}
```
...
| ------------------------- | ----------------- | ---------------------------------------- |
| `image`                   | required          | Docker image for sim pods                |
| `regcred`                 | —                 | Name of the Kubernetes `imagePullSecret` |
| `namespace`               | `"amesa-train"`   | Namespace for the training job           |
| `namespace_sims`          | `"amesa-sims"`    | Namespace for sim pods                   |
| `node_selector_agentpool` | `"simscpu"`       | Node pool label for pod scheduling       |
| `port_forward`            | `false`           | Port-forward sim pods to local machine (useful for remote cluster debugging) |
| `protocol`                | `"grpc"`          | Transport protocol                       |

### `amesa`

Runs training on AMESA-managed cloud.
...
| `plan`     | required | Compute plan: `"small"`, `"medium"`, or `"large"` |
| `protocol` | `"grpc"` | Transport protocol                                |

### `target.config` (optional, all targets)
...
| Field              | Default | Description                                     |
| ------------------ | ------- | ----------------------------------------------- |
| `sim_timeout`      | `300`   | Seconds before a sim step times out             |
| `watchdog_timeout` | `600`   | Seconds before the watchdog kills a stalled job |
...
See [perceptors.md](perceptors.md).
...
### `skills[]` — DRL skill (`type: "SkillTeacher"` or `"SkillTeacherGoal"`)

`"SkillTeacherGoal"` is a v1-only variant that drives reward from a declared goal rather than a manual `compute_reward` implementation. See [goals.md](goals.md).

Both types share the same config structure — `learning`, `resources`, and `model` blocks:
...
Coordinated skills (`SkillCoach` / `SkillCoordinatedSet`) are a v1-only feature. See [skills/coordinated.md](skills/coordinated.md).
...
**`local` target requires a running sim** — The trainer does not start the sim for you when using the `local` target. Start your sim server separately before submitting the job. See [simulator.md](simulator.md) for the server bootstrap pattern.
