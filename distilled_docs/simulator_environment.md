...
### Simulation Types: Custom vs. Data-Driven Models

AMESA supports two primary approaches to create simulations:

1. **Custom Simulators** (this document): For physics-based, phenomenological, or code-driven environments, use the ServerAMESA API.
2. **Data-Driven Models (DDM)** (no code): For systems with historical operational data, use AMESA's UI-based DDM approach. No coding required—upload a CSV and configure variables.

#### When to Use Data-Driven Models (DDM)

A **Data-Driven Model (DDM)** is ideal for industrial and logistics workflows where historical data reflects real-world constraints. Use DDM when:

- You have 3+ months of operational data (1 year preferred)
- Your data shows a >80% accurate relationship between control actions and sensor readings
- You want rapid simulation without physics modeling
- Your use case involves equipment behavior, logistics workflows, or process automation

#### DDM JSON Format

- The input to each model is: [previous obs] + [previous action] + [current action] in that order. The output is the single sensor's reading.
- Each JSON file corresponds to a single sensor reading for each step.
- The name of the sensor variable is related to the name of the JSON file.
